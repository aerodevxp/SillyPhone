...
export async function playBubbles(msgs, containerEl, side = 'char', ts = Date.now(), attachment = null, timing = null, isoTime = null) {
    if (reducedMotion()) {
        bubbles.appendBurst({ from: side, msgs, ts, attachment, isoTime }, containerEl);
        return;
    }
    ...
    const turn = bubbles.openTurn(side, ts, containerEl, attachment, isoTime);
